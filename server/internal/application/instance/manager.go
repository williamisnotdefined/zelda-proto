...
func (m *Manager) resolveTransfers() {
	for fromID, w := range m.worlds {
		for _, req := range w.ConsumeTransferRequests() {
			...
			// Transfers inherit the portal-system cooldown and then extend it through
			// a short post-transfer window, preventing bounce-backs.
			...
			// Per-instance on-enter hooks keep entry populations anchored to the
			// player's actual transfer location. Phase 3 must (re)anchor the entry
