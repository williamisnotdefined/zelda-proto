...
// shared TypeScript contract under shared/src/constants.ts.
//
// Constants here are duplicated from the shared package on purpose: keeping a
// Go copy keeps the build graph free of Node tooling. A future generator can
...
// Authoritative instance identifiers, mirroring shared/src/types.ts.
const (
...
// (matches the InstanceManager startup order in the legacy server).
