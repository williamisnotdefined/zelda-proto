...
import (
	...
	"github.com/williamisnotdefined/zelda-proto/server_go/internal/application/bossregion"
	appcombat "github.com/williamisnotdefined/zelda-proto/server_go/internal/application/combat"
	...
	"github.com/williamisnotdefined/zelda-proto/server_go/internal/application/spatial"
	"github.com/williamisnotdefined/zelda-proto/server_go/internal/application/spawn"
	...
	"github.com/williamisnotdefined/zelda-proto/server_go/internal/domain/combat"
...
// PlayerRespawnTime mirrors the legacy server constant. Kept as a re-export
// for backwards-compatible callers; the source of truth is
...
type Config struct {
	...
	// populations from the registry definition. The legacy zero-config path
...
// state. Mirrors InstanceManager.ensurePhase3BossesNear in the TS server.
...
// player's entry coordinates. Mirrors InstanceManager.ensurePhase3BossesNear
// in the TS reference server: removes any unexpected dragon ids, refreshes
...
func (w *World) EnsurePhase3BossesNear(entryX, entryY float64) {
	...
	for _, b := range w.def.Phase3EntryBosses {
		...
		if existing, ok := w.dragons[b.ID]; ok && existing.Kind() == b.Kind {
			...
			// future TryRespawn lands at the new entry. Don't teleport the
			// boss mid-fight (matches TS semantics).
			existing.SpawnX = x
			existing.SpawnY = y
			continue
		}
...
// when none is in range. Mirrors InstanceManager.ensurePhase2PopulationNear
// in the TS reference server. No-op for instances that aren't Phase 2 (i.e.
...
// around the given entry point if too few are alive nearby. Mirrors
// InstanceManager.ensurePhase4PopulationNear in the TS reference server.
...
func (w *World) tickHazards(dt time.Duration) {
	...
	// a hazard tile still take damage, mirroring server/src/game/HazardSystem.ts.
	const playerHalfDiag = 34.0
	hitR := hazard.HitRadius + playerHalfDiag
	hitR2 := hitR * hitR
	_ = hitR
	// Per-tick dedup so a player gets hit by at most one purple field per tick
	// (matches the TS implementation; otherwise stacked purples can chain-stun).
	...
	for id, h := range w.hazards {
		...
		for _, p := range w.players {
			...
			// Spawn-safezone protection: while the post-respawn invulnerability
			// window is active and the player stands inside the safe zone, no
			// hazard damage or burning ticks may be applied. Mirrors the check in
			// server/src/game/systems/HazardSystem.ts (resolveHazardDamage).
			if w.isProtected(p) {
				continue
			}
			if !h.MarkHit(p.ID) {
			...
			if physics.DistanceSquared(p.X, p.Y, h.X, h.Y) > hitR2 {
				delete(h.HitPlayerIDs, p.ID)
				continue
			}
			effect := hazard.EffectFor(h.Kind)
			if effect == hazard.EffectPurpleBurning {
				if _, dup := purpleHitThisTick[p.ID]; dup {
					continue
				}
				purpleHitThisTick[p.ID] = struct{}{}
			}
...
func (w *World) tickDrops() {
	...
	for _, e := range w.enemies {
		...
		if w.cfg.Rand.Float64() < drop.DropChance {
			...
			d := &drop.Drop{ID: id, X: e.X, Y: e.Y, Kind: e.DropKind}
			w.drops[id] = d
