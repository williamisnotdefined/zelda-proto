...
import (
	...
	appsnap "github.com/williamisnotdefined/zelda-proto/server_go/internal/application/snapshot"
...
// forceFullSnapshotEveryTicks mirrors server/src/network/WebSocketHandler.ts
// (SERVER_NET_TICK_RATE * 5) so we periodically resync clients with a full
// snapshot, guarding against any silent delta divergence.
...
// incremental afterwards) to mirror the TS SnapshotSerializer behaviour.
...
// snapshot, mirroring server/src/network/WebSocketHandler.ts.
...
// connection. Mirrors server/src/network/WebSocketHandler.ts buildLeaderboard.
...
// buildSnapshotEnvelope constructs the legacy full-snapshot envelope; used by
// tests and as a fallback. Production traffic flows through
